// Тестирование метода ANewDsc (вещественные корни - изолирующие интервалы)
...
int count_covered(const vector<T> &expected_roots,
	...
	for (auto &root : expected_roots) {
		for (auto &iv : intervals) {
			T lo=min(iv.first,iv.second);
			T hi=max(iv.first,iv.second);
...
TR run_test(const string &nm, unsigned P, unsigned ncl,
	...
	auto intervals=find_real_roots_ANewDsc(coef,eps);
	...
	r.ok=(r.covered>=int(r.expected*0.7))&&r.found>0;
...
void save(const vector<TR>&r,const string&fn){
	...
	o<<"========================================================================\n";
	o<<"ANewDsc (вещественные корни, изолирующие интервалы)\nТип: "<<(r.empty()?"":r[0].dtype)<<"\n";
	o<<"========================================================================\n\n";
...
#endif
	cout<<"=== ТЕСТ: ANewDsc ===\n";
	{stringstream l; auto r=run_all<double>(l); cout<<l.str(); save(r,"anewdsc_double.txt");}
	{stringstream l; auto r=run_all<long double>(l); cout<<l.str(); save(r,"anewdsc_long_double.txt");}
	{stringstream l; auto r=run_all<float_precision>(l); cout<<l.str(); save(r,"anewdsc_float_precision.txt");}
	cout<<"\nANewDsc — все тесты завершены.\n";
	return 0;
}
