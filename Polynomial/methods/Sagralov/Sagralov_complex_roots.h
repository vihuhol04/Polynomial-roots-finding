...
template <RealNumber T> struct Disk {
	...
	Disk(cplx<T> c = cplx<T>(0, 0), T r = T(0), int k = 0) : center(c), radius(r), num_roots(k) {}
};
...
std::optional<Disk<T>> NewtonTest(const std::vector<cplx<T>>& a, const Disk<T>& D, int N_C) {
	...
	// проверяем результат деления, а не делитель
...
std::vector<Disk<T>>
CIsolate(const std::vector<T_in>& coeffs_in, cplx<T> center = cplx<T>(0, 0), T halfw = static_cast<T>(0.0),
	T min_radius = numeric_constants::adaptive_epsilon<T>(numeric_constants::EPSILON_SCALE_COARSE),
	int max_iterations = numeric_constants::DEFAULT_MAX_ITERATIONS * 5)
{
	...
	// a posteriori контроль: если ведущий коэффициент слишком мал, полином вырожден
