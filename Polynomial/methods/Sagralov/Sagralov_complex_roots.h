...
// тест Пелле после N итераций Греффе
template <typename T>
int T_G(const std::vector<cplx<T>>& a, const cplx<T>& m, T r) {
    ...
    for (int iter = 0; iter <= N; ++iter) {
        ...
        for (size_t k = 0; k < mag.size(); ++k)
            if (mag[k] > K * (sum - mag[k])) return (int)k;

        // следующая итерация Греффе
        if (iter < N) {
...
// тест T* с переменным числом итераций Греффе
template <typename T>
...
std::optional<Disk<T>> NewtonTest(const std::vector<cplx<T>>& a, const Disk<T>& D, int N_C) {
    if (D.num_roots <= 0) return std::nullopt;
    ...
    if (abs_val(Fp_val) < T(1e-18)) return std::nullopt;

    cplx<T> x_new = x_C - cplx<T>(T(k), T(0)) * (F_val / Fp_val);
    T r_new = r / T(2 * N_C);

    int k_new = T_star(a, x_new, r_new);
    if (k_new == k) return Disk<T>(x_new, r_new, k);
...
std::vector<std::pair<cplx<T>, T>> BisectionSquare(const cplx<T>& center, T halfw) {
    ...
    return {
        {cplx<T>(center.real() - h2, center.imag() - h2), h2}, {cplx<T>(center.real() + h2, center.imag() - h2), h2},
        {cplx<T>(center.real() - h2, center.imag() + h2), h2}, {cplx<T>(center.real() + h2, center.imag() + h2), h2} };
}
...
std::vector<Disk<T>> CIsolate(const std::vector<T_in>& coeffs_in, cplx<T> center = cplx<T>(0, 0), T halfw = static_cast<T>(0.0),
    T min_radius = static_cast<T>(1e-3), int max_iterations = 500) {
    ...
    while (!work_queue.empty() && iteration < max_iterations) {
        ...
        if (nonempty_squares.empty()) continue;
    ...
    // обратная трансформация: диски для Q(y) → диски для P(x), делим центры и радиусы на k
    if (scale_k != T(1)) {
