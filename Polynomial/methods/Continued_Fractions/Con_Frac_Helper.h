...
// Интеграция в фреймворк + исправление ошибок + настройка шаблонности для пользовательских типов: Павлова Анастасия, КМБО-01-22
...
template<typename T>
T compute_partial_quotient(const std::vector<T>& coeffs) {
    if (coeffs.size() < 2) return T(1);

    // ищем первый ненулевой коэффициент
    size_t k = coeffs.size() - 1;
    while (k > 0 && abs(coeffs[k]) < 1e-10) k--;

    if (k == 0) return T(1);

    // берем отношение двух старших коэффициентов
    T a = coeffs[k] / coeffs[k - 1];
    return (a > 0) ? floor(a + 0.5) : ceil(a - 0.5); // округление до ближайшего целого
}
...
T build_convergent(const std::vector<T>& a, T& approx, double eps) {
    ...
    for (size_t k = 1; k < a.size(); ++k) {
        ...
        if (abs(approx - prev_approx) < eps) break;
        prev_approx = approx;
...
template<typename T>
std::vector<T> remove_rational_roots(std::vector<T>& coeffs, std::vector<T>& rational_roots, double eps = 1e-8) {
