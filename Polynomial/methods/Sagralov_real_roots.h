...
// Чистая консистентная версия без дубликатов.
...
std::vector<RootWithMultiplicity<T>> find_real_roots_by_Sagralov(const std::vector<T>& input, T epsilon = 1e-8)
{
    ...
    // Корень в нуле с кратностью (дефляция)
    int mult0 = 0;
    ...
    if (deg <= 3)
    {
        ...
        else if (deg == 3)
        {
            ...
            // Если всё ещё меньше степеней — простой скан c бисекцией.
        ...
        // Удаляем дубликаты (по модулю epsilon*10)
        std::sort(roots.begin(), roots.end());
        roots.erase(std::unique(roots.begin(), roots.end(), [&](T a, T b)
            { return abs_val(a - b) < T(epsilon * 10); }),
            roots.end());
        for (auto& r : roots)
        {
            T e = static_cast<T>(epsilon);
    ...
    // Границы по строгой формуле Коши M = 1 + max|a_i|/|a_n|; спец-кейс потолок=3 -> 4
    T maxc = T(0);
...
int detail::countSignChanges(const std::vector<std::vector<T>>& seq, T x)
{
    ...
    for (auto& p : seq)
    {
        T v = eval_poly(p, x);
        if (is_zero_val(v))
        { // минимальный сдвиг чтобы не терять знак на корне
...
// Кратность: порядок первой ненулевой производной, с усилением для тройной кратности.
template <typename T>
int detail::checkRootMultiplicity(const std::vector<T>& poly, T root, T eps)
{
    ...
    // Быстрая стабилизация центра
    for (int it = 0; it < 8; ++it)
    {
        T f = eval_poly(poly, root);
        T fp = eval_poly(d1, root);
        if (abs_val(f) < T(eps * 1e-4))
            break;
        if (abs_val(fp) < T(1e-22))
            break;
        T step = f / fp;
        if (abs_val(step) > T(0.5))
            step *= T(0.5);
        root -= step;
        if (abs_val(step) < T(1e-18))
            break;
    }
    ...
    auto near_zero = [&](T v, int ord)
        { T a = abs_val(v); T thr = T(eps) * scale * pow_val(T(1.7), ord); return a < thr; };
    ...
    // Усиление: если предполагается 2, проверяем шаблон тройного корня (P,P',P'' ~ 0, P''' !=0)
    if (mult == 2 && derivs.size() >= 4)
    {
        ...
        T v3 = eval_poly(derivs[3], root);
        if (near_zero(v0, 0) && near_zero(v1, 1) && near_zero(v2, 2) && !near_zero(v3, 3))
            mult = 3;
    }
    ...
    if (mult == 2 && derivs.size() >= 4)
    {
        ...
        T a2 = abs_val(v2);
        T a3 = abs_val(v3);
        // Условия кандидата: P≈0, P'≈0, P'' линейно мал и P''' заметен
    ...
    if (mult >= 3 && derivs.size() >= 4)
    {
        for (int it = 0; it < 10; ++it)
        {
            ...
            if (near_zero(f2, 2) || abs_val(f3) < T(1e-30))
                break;
            T step = f2 / f3;
            if (abs_val(step) > T(0.5))
                step *= T(0.5);
