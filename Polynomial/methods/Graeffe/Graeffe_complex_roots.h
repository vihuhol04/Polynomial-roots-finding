...
std::vector<std::complex<T>> hosman_modification_graeffe (const std::vector<T>& coeffs_desc, T epsilon = T(1e-10L), int maxIter = 100) {
    ...
    if (graeffe_moduli.empty()) return {}; // Греффе не нашёл модулей
    ...
    for (const auto& [r, multiplicity] : unique_moduli) {
        ...
        for (const auto& [a, from_grid] : candidate_reals) {
            ...
            if (b_squared < -epsilon) continue;
            if (b_squared < T(0)) b_squared = T(0);
            ...
            if (b_squared >= epsilon) {
                ...
                if (f_plus_abs > evaluation_threshold || f_minus_abs > evaluation_threshold) continue; 
            }
        ...
        std::sort(valid_roots.begin(), valid_roots.end(), [](const std::pair<T, T>& x, const std::pair<T, T>& y) { return x.second < y.second; });
    ...
    for (size_t i = 0; i < roots_with_residuals.size(); ++i) {
        ...
        if (!is_duplicate)
            ...
            std::complex<T> existing_root_t(unique_roots[best_existing_idx].real(), unique_roots[best_existing_idx].imag());
