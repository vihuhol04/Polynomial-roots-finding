...
// Вычисление (r² - x²)^k как полинома от x (ascending порядок)
template <typename T>
std::vector<T> power_of_binomial(T r_squared, int k)
{
    ...
    // Начинаем с (r² - x²)
    std::vector<T> result = { r_squared, T(0), T(-1) }; // r² + 0·x - x²
...
std::vector<std::complex<T>> hosman_modification_graeffe(
    ...
    int maxIter = 100)
{
    // Hosman modification called

    // ШАГ 1: Находим модули корней методом Греффе
    std::vector<T> graeffe_moduli = find_moduli_roots_by_graeffe(coeffs_desc, epsilon, maxIter);
    if (graeffe_moduli.empty())
    {
        return {}; // Graeffe не нашёл модулей
    }

    // Graeffe нашёл модули

    // Подсчитываем кратности модулей
    std::vector<T> moduli = graeffe_moduli;

    std::sort(moduli.begin(), moduli.end(), [](const T& a, const T& b)
        { return a < b; });
    ...
    while (i < moduli.size())
    {
        ...
        // Если найдём реальные корни - count используется напрямую
        // Если комплексные - count/2 пар a±bi
    ...
    // ШАГ 2: Для каждого уникального модуля применяем метод Хосмана
    for (const auto& [r, multiplicity] : unique_moduli)
    {
        ...
        // ШАГ 2.1: Построение уравнения Хосмана (1.5)
        // hosman_eq = f(x) - f''(x)·(r²-x²)/2! + f⁴(x)·(r²-x²)²/4! - ...
        ...
        // ШАГ 2.2: Находим кандидаты для x
