...
// Вычисление sqrt с fallback
template <typename T>
T sqrt_fallback(const T &x) {
...
template <typename T>
std::complex<double> eval_complex_poly(const std::vector<T> &coeffs_desc, const std::complex<double> &z) {
    if (coeffs_desc.empty())
        return std::complex<double>(0.0, 0.0);
    std::complex<double> result = std::complex<double>(to_double_fallback(coeffs_desc[0]), 0.0);
    for (size_t i = 1; i < coeffs_desc.size(); ++i)
        result = result * z + std::complex<double>(to_double_fallback(coeffs_desc[i]), 0.0);
    return result;
}
...
// Умножение двух полиномов (asc порядок)
template <typename T>
...
// Вычисление (r^2 - x^2)^k как полинома от x (asc)
template <typename T>
std::vector<T> power_of_binomial(T r_squared, int k) {
    if (k == 0) return {T(1)};

    // Начинаем с (r^2 - x^2)
    std::vector<T> result = {r_squared, T(0), T(-1)}; // r^2 + 0x + x^2
...
// Основная функция
template <typename T>
std::vector<std::complex<T>> hosman_modification_graeffe(const std::vector<T> &coeffs_desc,
    T epsilon = from_long_double<T>(1e-10L), int maxIter = 100)
{
    // Находим модули корней методом Грефе
    std::vector<T> graeffe_moduli = find_moduli_roots_by_graeffe(coeffs_desc, epsilon, maxIter);
    if (graeffe_moduli.empty()) { return {}; }

    // Подсчитываем кратности модулей
    std::vector<T> moduli = graeffe_moduli;
    std::sort(moduli.begin(), moduli.end(), [](const T &a, const T &b) { return to_double_fallback(a) < to_double_fallback(b); });

    // Группируем модули: считаем количество для каждого уникального значения
    std::vector<std::pair<T, int>> unique_moduli; // (модуль, кол-во)
    size_t i = 0;
    while (i < moduli.size()) {
        T current_mod = moduli[i];
        int count = 0;
        size_t start_i = i;
        // Считаем все модули близкие к current_mod (порог больше для накопленных ошибок Грефе)
        while (i < moduli.size() && abs_val(moduli[i] - current_mod) < T(1e-7)) {
            count++;
            i++;
        }
        // НЕ делим на 2 здесь! Это будет решаться при обработке
        // Если найдём реальные корни - count используется напрямую, если комплексные - count/2 пар a +- bi
    ...
    for (size_t ord = 1; ord <= deg; ++ord) {
        ...
        if (derivatives.back().empty()) break;
    }

    // Вычисляем масштаб полинома 
    T max_coeff = T(0);
    for (const auto &c : coeffs_desc) max_coeff = std::max(max_coeff, abs_val(c));
    ...
    for (const auto &[r, multiplicity] : unique_moduli) {
        ...
        T r_squared = r * r;
        T local_scale_factor = max_coeff * std::pow(to_double_fallback(r), 2.0);
        if (to_double_fallback(local_scale_factor) < 1.0)
            local_scale_factor = T(1.0);
        ...
        if (to_double_fallback(local_scale_factor) > 1e6)
            base_resid_mult = base_resid_mult * T(1000000); // дополнительно 1e6
        else if (to_double_fallback(local_scale_factor) > 1e3)
        ...
        for (int k = 2; k < static_cast<int>(derivatives.size()); k += 2) {
            ...
            for (auto &coef : term) coef = coef / fact_k;

            // Знак: (-1)^(k/2)
            if ((k / 2) % 2 == 1)
                for (auto &coef : term) coef = -coef;

            hosman_eq = add_polynomials(hosman_eq, term);
        }
        // Hosman equation построено
        // Переводим в desc для find_moduli_roots_by_graeffe
        ...
        if (abs_val(val_at_zero) < evaluation_threshold) candidate_reals.push_back({T(0), false});

        // Находим модули корней уравнения Хосмана
        std::vector<T> candidate_moduli = find_moduli_roots_by_graeffe(hosman_eq_desc, epsilon);

        // Модули Hosman equation найдены
        // Тестируем +- x_mod
        ...
        // Если ни один кандидат не прошёл
        if (candidate_reals.empty()) {
            int num_samples = 100;
            T grid_threshold = evaluation_threshold * T(10); // В 10 раз мягче
        ...
        if (candidate_reals.empty()) continue; // Нет кандидатов для данного r

        // Валидация кандидатов уравнением (1.6)
        std::vector<std::pair<T, T>> valid_roots;

        for (const auto &[a, from_grid] : candidate_reals) {
            T b_squared = r_squared - a * a;
            if (to_double_fallback(b_squared) < -to_double_fallback(epsilon)) continue;
            if (to_double_fallback(b_squared) < 0.0) b_squared = T(0);

            // Для комплексных корней: проверяем f(a+-bi) на исходном полиноме
            if (to_double_fallback(b_squared) >= to_double_fallback(epsilon)) {
                T b = sqrt_fallback(b_squared);
                std::complex<double> z_plus(to_double_fallback(a), to_double_fallback(b));
                std::complex<double> z_minus(to_double_fallback(a), -to_double_fallback(b));

                double f_plus = std::abs(eval_complex_poly(coeffs_desc, z_plus));
                double f_minus = std::abs(eval_complex_poly(coeffs_desc, z_minus));

                double eval_thr = to_double_fallback(evaluation_threshold);

                if (f_plus > eval_thr || f_minus > eval_thr) continue;
            }

            T abs_residual;
            T current_threshold = from_grid ? relaxed_residual_threshold : residual_threshold;

            // Для вещественных корней (b^2 =(примерно) 0) проверяем f(a)=0
            if (to_double_fallback(b_squared) < to_double_fallback(epsilon)) {
                // Вещественный корень
            ...
            if (abs_residual < current_threshold) valid_roots.push_back({a, abs_residual});
        }

        // Если после валидации не нашли корней
        if (valid_roots.empty()) {
            ...
            // Если вещественных не нашли, продолжаем для комплексных
            if (valid_roots.empty()) {
                int num_samples = 100;
                T grid_eval_threshold = evaluation_threshold * T(1000); 
                T grid_resid_threshold = relaxed_residual_threshold;    

                for (int i = 1; i <= num_samples; ++i) {
                    T a_test = r * T(i) / T(num_samples + 1);
                    T b_squared = r_squared - a_test * a_test;

                    if (to_double_fallback(b_squared) < 0.0) continue;
                    ...
                    if (abs_val(val_pos) < grid_eval_threshold) {
                        // Если b^2 мало, возможен вещественный корень +- r (уже проверили выше)
                        if (to_double_fallback(b_squared) >= to_double_fallback(r * epsilon)) {
                            T b = sqrt_fallback(b_squared);
                            std::complex<double> z_plus(to_double_fallback(a_test), to_double_fallback(b));
                            std::complex<double> z_minus(to_double_fallback(a_test), -to_double_fallback(b));
        ...
        if (valid_roots.empty()) continue;

        // Сортируем по residual и добавляем все валидные корни
        std::sort(valid_roots.begin(), valid_roots.end(), [](const std::pair<T, T> &x, const std::pair<T, T> &y) { return to_double_fallback(x.second) < to_double_fallback(y.second); });
    ...
    // Улучшенная дедупликация с выбором по минимальной невязке
    ...
    for (const auto &root : roots) {
        std::complex<double> root_double(to_double_fallback(root.real()), to_double_fallback(root.imag()));
        auto eval = eval_complex_poly(coeffs_desc, root_double);
        T residual = T(std::abs(eval));
    ...
    for (size_t i = 0; i < roots_with_residuals.size(); ++i) {
        ...
        if (!is_duplicate)
            ...
            std::complex<double> existing_root_double(to_double_fallback(unique_roots[best_existing_idx].real()), to_double_fallback(unique_roots[best_existing_idx].imag()));
            auto current_eval = eval_complex_poly(coeffs_desc, existing_root_double);
            T current_residual = T(std::abs(current_eval));
 ...
 */
template <typename T>
std::vector<std::complex<T>> hosman_with_multiplicities(const std::vector<T> &coeffs_desc, const std::vector<ModulusWithMultiplicity<T>> &moduli_with_multiplicities, T epsilon = from_long_double<T>(1e-10L))
{
    if (moduli_with_multiplicities.empty()) { return {}; }
    ...
    for (int deriv_order = 1; deriv_order <= static_cast<int>(deg); ++deriv_order) {
        ...
        for (size_t i = 1; i < prev.size(); ++i) current_deriv[i - 1] = prev[i] * T(i);
    ...
    for (const auto &mod_with_mult : moduli_with_multiplicities) {
        ...
        if (to_double_fallback(r) < to_double_fallback(epsilon)) {
        ...
        for (int k = 1; k <= static_cast<int>(deg) / 2; ++k) {
            int deriv_order = 2 * k;
            if (deriv_order >= static_cast<int>(derivatives.size())) break;
            if (derivatives[deriv_order].empty()) break;
        ...
        for (const auto &[a, from_grid] : candidate_reals) {
            ...
            if (to_double_fallback(b_squared) >= to_double_fallback(epsilon)) {
                ...
                if (f_plus <= eval_thr) valid_candidates.push_back({std::complex<T>(a, b), T(f_plus)});
                if (f_minus <= eval_thr) valid_candidates.push_back({std::complex<T>(a, -b), T(f_minus)});
            } else {
                // Вещественный корень
                T f_a = eval_poly(derivatives[0], a);
                ...
                if (abs_residual < current_threshold) valid_candidates.push_back({std::complex<T>(a, T(0)), abs_residual});
            }
        }
        ...
        std::sort(valid_candidates.begin(), valid_candidates.end(), [](const CandidateRoot &a, const CandidateRoot &b) { return to_double_fallback(a.residual) < to_double_fallback(b.residual); });
